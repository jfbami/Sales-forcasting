...
This project forecasts the next 16 days of sales for 33 product families across 54 stores. The work is split into two stages:
...
The exploratory analysis is not an afterthought. Every modelling choice in the pipeline (lags, payday flags, oil price lag, the 2016 earthquake window) comes straight from a pattern visible in one of the plots below.

## Exploratory Analysis

The plots tell the story of the data. They live in [`plots/`](plots) and are the foundation for the engineered features.
...
- **Calendar.** Day of week, month, year, week, weekend, month start/end, payday (15th and last), New Year, Christmas season, post-earthquake window.
- **Lags.** Sales lags from 16 to 56 days and 364 days (kept at 16+ to avoid leakage into the test horizon), plus 1 to 4 week day-of-week lags.
- **Rolling statistics.** 7, 14, 28 and 91 day rolling means and standard deviations.
...
Predictions are combined with inverse-RMSLE weighting, so the better validation models get higher weight.
...
3. **Validation** on the final 16 days, which mirrors the test horizon.
...
# 4. Train and predict, produces submission.csv
python pipeline.py
```
